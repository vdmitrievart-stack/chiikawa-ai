  ...
  maybeOpenPosition({ plan, candidate, heroImage, walletId }) {
    return portfolioOpenPosition({
      ...
      signalContext: {
        ...
        migrationAccumulation: candidate.migrationAccumulation,
        holderAccumulation: candidate.holderAccumulation,
        runnerLike: candidate.runnerLike,
        packaging: candidate.packaging,
        reversal: candidate.reversal,
        scalp: candidate.scalp,
        corpse: candidate.corpse,
        falseBounce: candidate.falseBounce,
        copytradeMeta: candidate.copytradeMeta,
        ...
        tokenSnapshot: {
          ...
          dexId: candidate?.token?.dexId || "",
          price: safeNum(candidate?.token?.price, 0),
          liquidity: safeNum(candidate?.token?.liquidity, 0),
          fdv: safeNum(candidate?.token?.fdv, 0),
          volume: safeNum(candidate?.token?.volume, 0),
          volumeH1: safeNum(candidate?.token?.volumeH1, 0),
          volumeH24: safeNum(candidate?.token?.volumeH24, candidate?.token?.volume || 0),
          txnsH1: safeNum(candidate?.token?.txnsH1, 0),
          txnsH24: safeNum(candidate?.token?.txnsH24, candidate?.token?.txns || 0),
          pairCreatedAt: safeNum(candidate?.token?.pairCreatedAt, 0),
        ...
        strategyMetrics: {
          ...
          migrationAccumulationScore: safeNum(candidate?.migrationAccumulation?.score, 0),
          runnerScore: safeNum(candidate?.runnerLike?.score, 0),
          packagingScore: safeNum(candidate?.packaging?.score, 0),
          holderControlPct: safeNum(candidate?.holderAccumulation?.netControlPct, 0),
          quietAccumulation: Boolean(candidate?.holderAccumulation?.quietAccumulationPass),
          corpseRisk: Boolean(candidate?.corpse?.isCorpse),
          falseBounceRisk: Boolean(candidate?.falseBounce?.rejected),
          migrationStructure: Boolean(candidate?.migration?.passes),
          migrationAccumulation: Boolean(candidate?.migrationAccumulation?.priorityWatch || candidate?.migrationAccumulation?.allow),
          runnerStructure: Boolean(candidate?.runnerLike?.allow),
          copySoftWarning: Boolean(candidate?.copytradeMeta?.softWarning),
          dexPaid: Boolean(candidate?.dexPaid?.paid),
          devVerdict: String(candidate?.developer?.verdict || "Unknown"),
          socialsCount: Array.isArray(candidate?.socials) ? candidate.socials.length : 0
        }
      },
      walletId,
