...
export default class BotRouter {
  ...
  createSendBridge(chatId) {
    return {
      text: (text, extra = {}) => this.sendMessage(chatId, text, { reply_markup: this.keyboard(), ...extra }),
      photoOrText: (imageUrl, caption, extra = {}) =>
        this.sendPhotoOrText(chatId, imageUrl, caption, { reply_markup: this.keyboard(), ...extra })
    };
  }
  ...
  startLoop(chatId, userId) {
    ...
    if (this.kernel.getRuntime().mode === "4h") {
      this.stopTimeoutId = setTimeout(async () => {
        ...
        await this.sendMessage(chatId, this.t("soft_stop"), { reply_markup: this.keyboard() });
      }, 4 * 60 * 60 * 1000);
    }
  }
  ...
  async exportJson(chatId) {
    const filePath = path.join(os.tmpdir(), `chiikawa-stats-${this.kernel.getRuntime().runId || Date.now()}.json`);
  ...
  async exportCsv(chatId) {
    const filePath = path.join(os.tmpdir(), `chiikawa-stats-${this.kernel.getRuntime().runId || Date.now()}.csv`);
  ...
  async exportXlsx(chatId) {
    const filePath = path.join(os.tmpdir(), `chiikawa-stats-${this.kernel.getRuntime().runId || Date.now()}.xlsx`);
  ...
  async processStatefulInput(chatId, text) {
    ...
    if (mode.mode === "awaiting_ca") {
      if (!isLikelyCA(text)) {
        await this.sendMessage(chatId, this.t("invalid_ca"), { reply_markup: this.keyboard() });
        return true;
      }
    ...
    if (mode.mode === "awaiting_secret_ref") {
      ...
      if (!match) {
        await this.sendMessage(chatId, this.t("secret_format"), { reply_markup: this.keyboard() });
        return true;
      }
      ...
      if (!ok) {
        await this.sendMessage(chatId, this.t("wallet_not_found"), { reply_markup: this.keyboard() });
        return true;
      }

      this.clearChatMode(chatId);
      await this.sendMessage(chatId, `${this.t("secret_saved")}\n<b>${walletId}</b> → <code>${secretRef}</code>`, {
        reply_markup: this.keyboard()
      });
      return true;
    }
  ...
  async handleAction(chatId, userId, action) {
    if (action === "start") {
      this.clearChatMode(chatId);
      await this.sendMessage(chatId, this.t("ready"), { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "run_multi") {
      ...
      await this.sendMessage(chatId, `${this.t("run_started")}: MULTI`, { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "run_scalp") {
      ...
      this.startLoop(chatId, userId);
      await this.sendMessage(chatId, `${this.t("run_started")}: SCALP`, { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "run_reversal") {
      ...
      this.startLoop(chatId, userId);
      await this.sendMessage(chatId, `${this.t("run_started")}: REVERSAL`, { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "run_runner") {
      ...
      this.startLoop(chatId, userId);
      await this.sendMessage(chatId, `${this.t("run_started")}: RUNNER`, { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "run_copytrade") {
      ...
      this.startLoop(chatId, userId);
      await this.sendMessage(chatId, `${this.t("run_started")}: COPYTRADE`, { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "kill") {
      ...
      this.stopLoop();
      await this.sendMessage(chatId, `${this.t("hard_kill")}\nclosed: ${closed.length}`, {
        reply_markup: this.keyboard()
      });
      return;
    }
    ...
    if (action === "status") {
      await this.sendMessage(chatId, this.kernel.buildStatusText(), { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "scan_market") {
      await this.sendMessage(chatId, "🔎 <b>Market scan started</b>", { reply_markup: this.keyboard() });
    ...
    if (action === "scan_ca") {
      ...
      await this.sendMessage(chatId, this.t("send_ca"), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "language") {
      await this.sendMessage(chatId, this.t("choose_lang"), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "lang_ru") {
      this.kernel.setLanguage("ru");
      await this.sendMessage(chatId, `${this.t("lang_set")}: RU`, { reply_markup: this.keyboard() });
      return;
    }

    if (action === "lang_en") {
      this.kernel.setLanguage("en");
      await this.sendMessage(chatId, `${this.t("lang_set")}: EN`, { reply_markup: this.keyboard() });
      return;
    }

    if (action === "wallets") {
      await this.sendMessage(chatId, this.kernel.buildWalletsText(), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "copytrade") {
      await this.sendMessage(chatId, this.kernel.buildCopytradeText(), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "budget") {
      await this.sendMessage(chatId, this.kernel.buildBudgetText(), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "gmgn_status") {
      await this.sendMessage(chatId, this.kernel.buildGmgnStatusText(), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "leader_health") {
      await this.sendMessage(chatId, await this.kernel.buildLeaderHealthText(), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "sync_leaders") {
      await this.kernel.syncLeaderScores();
      await this.sendMessage(chatId, this.t("leaders_synced"), { reply_markup: this.keyboard() });
      await this.sendMessage(chatId, await this.kernel.buildLeaderHealthText(), { reply_markup: this.keyboard() });
      return;
    }
    ...
    if (action === "add_leader") {
      ...
      await this.sendMessage(chatId, this.t("add_leader_prompt"), { reply_markup: this.keyboard() });
      return;
    }

    if (action === "set_secret") {
      this.setChatMode(chatId, "awaiting_secret_ref");
      await this.sendMessage(chatId, this.t("add_secret_prompt"), { reply_markup: this.keyboard() });
      return;
    }
    ...
    await this.sendMessage(chatId, this.t("unknown"), { reply_markup: this.keyboard() });
  }
  ...
  async handleMessage(msg) {
    ...
    if (budgetMatch) {
      ...
      await this.sendMessage(chatId, `${this.t("pending_budget_saved")}
...
${Math.round(result.budget.scalp * 100)} / ${Math.round(result.budget.reversal * 100)} / ${Math.round(result.budget.runner * 100)} / ${Math.round(result.budget.copytrade * 100)}`, {
    ...
    if (isLikelyCA(text)) {
      await this.sendMessage(chatId, this.t("scan_hint"), { reply_markup: this.keyboard() });
      return;
    }
