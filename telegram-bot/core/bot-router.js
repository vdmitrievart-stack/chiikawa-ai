...
export default class BotRouter {
  ...
  async runScanCA(chatId, ca) {
    await this.sendMessage(chatId, "🔎 <b>CA scan started — ROUTER V13 ACTIVE</b>", {
  ...
  async runTeamScan(chatId, ca) {
    await this.sendMessage(chatId, "🕵️ <b>Team / Insider / Sniper scan started — ROUTER V13 ACTIVE</b>", {
      reply_markup: this.keyboard()
    });
    ...
    try {
      ...
      await this.sendMessage(
        chatId,
        `❌ <b>Team scan error — ROUTER V13</b>\n<code>${String(error?.message || error).slice(0, 900)}</code>`,
        { reply_markup: this.keyboard() }
      );
    }
  }
...
migration_survivor: ${(result.budget.migration_survivor * 100).toFixed(1)}%`,
    ...
    if (isLikelyCA(text)) {
      ...
      // Team scan still works through the persisted pending state after pressing 🕵️ Team Scan V13,
