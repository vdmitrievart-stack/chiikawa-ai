...
function buildScalpPlan(analyzed) {
  const { token, delta, accumulation, distribution, corpse, developer, strategy } = analyzed;
  ...
  if (strategy.expectedEdgePct < costs + 1.0) return null;
  ...
  return {
    ...
    runnerTargetsPct: [],
    expectedEdgePct: strategy.expectedEdgePct
  };
}

function buildReversalPlan(analyzed) {
  const { token, delta, absorption, accumulation, corpse, developer } = analyzed;
  const costs = estimateRoundTripCostPct();

  if (corpse.isCorpse) return null;
  if (developer.verdict === "Bad") return null;
  if (token.liquidity < 12000) return null;
  if (!delta.hasHistory) return null;
  ...
  const expectedEdge = Math.max(4.5, safeNum(analyzed.strategy.expectedEdgePct, 0) + 1.2);
...
function buildRunnerPlan(analyzed) {
  const { token, delta, absorption, accumulation, sentiment, developer, corpse } = analyzed;
  ...
  return {
    ...
    expectedEdgePct: Math.max(8, safeNum(analyzed.strategy.expectedEdgePct, 0) + 3)
  };
}
