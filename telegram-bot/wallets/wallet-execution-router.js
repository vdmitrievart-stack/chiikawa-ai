...
export default class WalletExecutionRouter {
  constructor(options = {}) {
    ...
    this.secretResolver = options.secretResolver || new WalletSecretResolver({ logger: this.logger });
    this.executionFactory = options.executionFactory || ((cfg) => new Level5ExecutionEngine(cfg));
    this.cache = new Map();
  }
  ...
  validateWalletForStrategy(runtimeConfig, walletId, strategyKey) {
    ...
    if (Array.isArray(wallet.allowedStrategies) && !wallet.allowedStrategies.includes(strategyKey)) {
  ...
  buildExecutionEngine(runtimeConfig, walletId) {
    ...
    try {
      const engine = this.executionFactory({
        ...
        isDryRun: !live,
        secretKeyBase58: live ? secretKeyBase58 : process.env.SOLANA_PRIVATE_KEY_BASE58 || "11111111111111111111111111111111"
      });
